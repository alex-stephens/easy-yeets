...
class InstagramScraper(object):
    ...
    def scrape_location(self):
        return self.__scrape_query(self.query_location_gen)
    ...
    def search_locations(self):
        ...
        p_id = result['places'][0]['place']['location']['pk']

        return p_id
#        place = sorted_places['place']
...
def main():
    ...
    list_dict = pickle.load(open("rest__.txt", "rb"))
    ...
    for i in reversed(range(len(list_dict))):
        ...
        try:
            ...
            scraper.usernames = [d['location_id']]
            urlarr = scraper.scrape_location()
            d['image_url'] = urlarr
        except:
            list_dict.pop(i)

    print(len(list_dict))
    with open("rest_url.txt", "wb") as myfile:
        pickle.dump(list_dict, myfile, protocol=2)


    # myDicts = pickle.load(open("restaurants_pid.txt", "rb"))
    # print(myDicts)
    #
    # scraper.usernames = [item['place_id'] for item in myDicts]
