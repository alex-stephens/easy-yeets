...
def main():
    ...
    list_dict = pickle.load(open("rest_.txt", "rb"))
    ...
    for i in reversed(range(len(list_dict))):
        ...
        try:
            d = list_dict[i]
            scraper.usernames = d['name'].split(' ')
            loc = scraper.search_locations()
            print(loc)
            if(loc == '0'):
                list_dict.pop(i)
                continue
    ...
    print(len(list_dict))
    with open("rest__.txt", "w") as myfile:
